...
for i = 2:num_sim
    ...
    for j = 1:num_agent
        ...
        est_traj_mean.castt.agent(j).x = est_traj_mean.castt.agent(j).x + data_save(i).castt.agent(j).x;
    end
end
...
for i = 1:num_agent
    ...
    est_traj_mean.castt.agent(i).x = est_traj_mean.castt.agent(i).x/num_sim;
end
...
title('STT','Position',[13.5,20,45])

subplot('position',subplot_position(4,:))
hold on
for j = 1:num_agent
    plot3(est_traj_mean.castt.agent(j).x(1,:),est_traj_mean.castt.agent(j).x(2,:),est_traj_mean.castt.agent(j).x(3,:),'--','color',[0.60,0.20,0.80],'LineWidth',2);
    plot3(est_traj_mean.castt.agent(j).x(1,end),est_traj_mean.castt.agent(j).x(2,end),est_traj_mean.castt.agent(j).x(3,end),'^','color',[0.15,0.59,0.38],'LineWidth',2);
end
title('confidence-aware-STT','Position',[13.5,20,45])
